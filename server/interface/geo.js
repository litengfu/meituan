...
const sign='4d90125d14baa2c10e0d3346e04c7a7b';
    ...
    if(!result.length){
      ...
      if (status === 200) {
        let position=new Position({
          province:province,
          city:ctx.query.city?ctx.query.city:city
        })
        position.save()
        ctx.body = {
          province,
        ...
        if(ctx.query.province&&ctx.query.city){
           await Position.where({city:result[0].city}).update({province:ctx.query.province,city:ctx.query.city}).where({province:result[0].province})
           ctx.body={
